#!/bin/bash
#Parameters
impl="tester-3200MHz-2-4-6-8-10G-S-3-lwB4-4-Fg-100-DUT-XDP-FLR" # name of the tested implementation (used for logging)
dir="b" # valid values: b,f,r; b: bidirectional, f: forward (Left to Right, 6 --> 4), r: reverse (Right to Left, 6 <-- 4) 
rate_start=250000 # starting from this packet rate
rate_end=2250000 # ending at this packet rate
rate_step=250000 # increase of the for cycle: for (( r=rate_start; r <= rate_end; r+=rate_step ))
...
do
	...
	do 
		...
		echo "Command line is: ./build/lwaftr-tp $fs $r $xpts $to $n $m"
		# Execute the test program
                ./build/lwaftr-tp $fs $r $xpts $to $n $m > temp.out 2>&1
